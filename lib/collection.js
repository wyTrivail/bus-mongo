...
var Collection = function(db_name, collection_name, collection_opt){
    ...
    this.pre_handle_update_args = function(args){
        ...
        if(this.collection_options.enable_date){
            if(new_args.payment.$set) new_args.payment.$set.updated_at = new Date();
...
Collection.prototype.find = function(){
    ...
    this.get_collection((err, collection) => {
        ...
        else if(Object.keys(args).length === 2) collection.find(args.query).toArray(args.callback);
        else if(Object.keys(args).length === 3) collection.find(args.query, args.options).toArray(args.callback);
    });
};
...
Collection.prototype.count = function(){
    ...
    this.get_collection((err, collection) => {
        if(err) args.callback(err);
        else collection.find(args.query).count(args.callback);
    });
};
